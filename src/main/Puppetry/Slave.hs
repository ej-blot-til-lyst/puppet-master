...
module Puppetry.Slave ( Action, serve ) where
...
type Action   = Text.Text -> Text.Text
type ClientId = Int
...
wsApp :: Action -> Concurrent.MVar State -> WS.ServerApp
wsApp action stateRef pendingConn = do
  conn <- WS.acceptRequest pendingConn
  clientId <- connectClient conn stateRef
  WS.forkPingThread conn 30
  Exception.finally
    (listen action conn clientId stateRef)
